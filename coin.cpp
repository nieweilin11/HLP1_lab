...
#include <cctype>
 ...
 vector<int> coin_flip(vector<int> x, int y[3],int b) {
    for (int i = 0; i < b; i++) {//将编号数组代入coin 进行判断
        if (x[y[i] - 1] == 1) //1为正面0为反面
            x[y[i] - 1] = 0;
        else if (x[y[i] - 1] == 0) {
            if (x[y[b - 1]-1] == 0 && b > 1) {//当编号最大的为背面时则那个硬币将不能翻转
                for (int i = 0; i < b; i++) {
                    y[i] = 4;
                    cout << "y" << i << "=" << y[i];
                }
...
int main(){
    ...
    while (sum_total)
    {
        ...
        if (round_counter % 2 == 0&& round_counter!=0) {//回合计数器
            round++;
            cout << endl;
            cout << endl;
            cout << endl;
        ...
        cout << "                                           " << " " << " " << " " << " "<< " " << " " << " " << " " << " " << " " << " " << " "<<"[" << player[n] << "]" << endl;
         ...
         try {
             ...
             //cout << endl;
             //int judge = 0;
             //if (!isdigit(input_choose)) { //判断是否为数字
             //    judge = 1;
             //}
             string str;
             cin >> str;
            ...
            /* if (judge > 0) {
             ...
             if (select == 3) {
                 ...
                 error("Please enter  number  ");
                    ...
                    for (int i = 0; i< coin_size; i++) {
                        len[i] = coin_num[i];//用数组来储存每个vector的
                    }
                    ...
                    coin_num.clear();
                    int coin_cout_p = 0;
                    for (int i = 0; i < 10; i++) { //头和尾的判断
                        if (coin[i] == 1)
                            coin_cout_p ++;
                    }

                    sum_total = coin_cout_p ;//最后相加
                    if (sum_total == 0)
                        break;
                    n++;
                    if (n > 1)n = 0;
             ...
             for (int i = 0; i < coin_num.size(); i++)
                 len[i] = 4;
             cout << "                                          "<<e.what() << endl;
             if (round_counter>0&&round > 1 && round_counter % 2 == 0)//当执行一次报错时，防止回合计数器再加一次
                 round_counter--;
             cout << endl;
             cout << endl;
             continue;
